...
import os
import numpy as np
from model_single import ModelEmb, OurBigModelEmb
...
def train_single_epoch(ds, model, sam, optimizer, transform, epoch, writer):
    loss_list = []
    pbar = tqdm(ds)
    ...
    for ix, (imgs, gts, original_sz, img_sz) in enumerate(pbar):
        ...
        # Log image grids to TensorBoard every 10 batches
        if ix % 10 == 0:
            # Create grids
            img_grid = make_grid(orig_imgs)
            mask_grid = make_grid(masks)

            # Log the grids
            writer.add_image(f'Input/epoch_{epoch}_batch_{ix}', img_grid, epoch)
            writer.add_image(f'Output/epoch_{epoch}_batch_{ix}', mask_grid, epoch)
...
def main(args=None, sam_args=None):
    ...
    writer = SummaryWriter(log_dir=os.path.join('runs', f'tensorboard_{args["folder"]}'))

    best = 0
    path_best = 'results/gpu' + str(args['folder']) + '/best.csv'
    f_best = open(path_best, 'w')
    for epoch in range(int(args['epoches'])):
        train_single_epoch(train_data, model.train(), sam.eval(), optimizer, transform, epoch, writer)
        with torch.no_grad():
            IoU_val = inference_ds(valid_data, model.eval(), sam, transform, epoch, args)
            if IoU_val > best:
...
if __name__ == '__main__':
    ...
    sam_args = {
        'sam_checkpoint': "segment_anything/sam_vit_h.pth",
        'model_type': "vit_h",
        'generator_args': {
            'points_per_side': 8,
