...
class CamObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_size = tuple(image.shape[1:3])
        #edge = cv2.dilate(edge, self.kernel, iterations=1) # FIXME
        #edge = Image.fromarray(edge)
        #edge = self.flip(edge)
        #edge = self.ge_transform(edge)
        return image, gt, torch.Tensor(
...
class test_dataset:
    ...
    def __getitem__(self, index):
        image = self.rgb_loader(self.images[index])
        original_size = torch.tensor(image.size)
        gt = self.binary_loader(self.gts[index])
        # Apply transformations
        image = self.transform(image)
        gt = self.gt_transform(gt)
        image_size = torch.tensor(image.shape[1:3])

        return image, gt, original_size, image_size
...
def get_test_loader(image_root, gt_root, testsize, shuffle=True, num_workers=4, pin_memory=True):
    dataset = test_dataset(image_root, gt_root, testsize)
    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=1,
                                  shuffle=shuffle,
                                  num_workers=num_workers,
